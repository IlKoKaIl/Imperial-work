...
The cause is cold dust. At a fixed infrared luminosity, the model star-forming spectra
peak near 126 micrometres, while observations place the peak closer to 92 micrometres.
The model peak also barely changes as luminosity increases. Colder dust shifts too much
energy to the longest wavelengths, overfilling the 350 and 500 micrometre bins.
...
match the light it was trained on but has partial failures in the parts it was not trained on.
...
photometry from the COSMOS field after being trained on optical and near-infrared data. Its far-infrared (FIR) output is generated by energy balance in the
stellar population synthesis model, this is done by fixing the total infrared luminosity \LIR{}.
But this extrapolation does not control how that luminosity is distributed over
FIR wavelength. This work tests that extrapolation against \emph{Herschel}/SPIRE
observations at 250, 350 and 500{\um}. Nine published differential-count products
...
The baseline \popcosmos{}/FSPS catalogue over-predicts the observed counts in every band.
Across 10--\SI{30}{mJy}, the median excess factors are 1.24, 1.72 and 2.29 at 250, 350
and \SI{500}{\um}; across 30--\SI{100}{mJy} they rise to 1.67, 1.69 and 3.57. The polarity
...
$[+0.16,+0.52]$, which is used as robustness sanity checked as opposed to a formal confidence interval

The physical diagnostic shows that low-AGN model galaxies are both too cold and too
uniform. At $L_\mathrm{IR}=10^{12}\,L_\odot$, their median FIR peak is
...
the direction of the correction but do not help select a dust model. A separate short-wavelength
peak near \SI{14}{\um} is associated with the model AGN parameter and is hidden
...
Jin agree with each other to only 0.12--\SI{0.15}{dex}, showing that deblending cannot
account for most of this spread. In a steep flux distribution, the model scatter moves
many more faint galaxies up into brighter bins than the opposite. A decomposition on the
...
0.20 and \SI{0.15}{dex} at 350 and \SI{500}{\um}, where the cold SED remains important.
The main suggestion is to keep the learned population and energy balance,
...
A disagreement here
is useful to study because it probes physics that the calibration data could not
constrain.
...
These tests gave useful context but could not cleanly determine whether \popcosmos{}
was wrong. The ambiguity motivated the change to observed flux densities and number
...
and were converted to micrometres throughout the entire project. All SEDs used in this
...
\popcosmos{} (0.42--\SI{0.51}{dex}). This allows the large per-object scatter to be isolated to
...
versus those just promoted by scatter. The mechanism is mathematically analogous to
...
relationship and found no redshift evolution out to $z\sim2$. For each low-AGN
($f_\mathrm{AGN}<0.1$) \popcosmos{} galaxy, the rest-frame FIR peak wavelength is
measured and plotted against infrared luminosity. This tests whether the model dust is
too cold, too uniform, or both.
...
Before testing the far-infrared predictions, it should first be confirmed that \popcosmos{} works well within its training regimes. Predicted and observed IRAC magnitudes were compared for all galaxies with coverage in each channel.
...
confirm that the catalogue, the SED products and the comparison code are all working
correctly. Ch3 and Ch4 have lower coverage and are therefore weaker tests.
...
magnitude across the full redshift range. This shows that the far-infrared discrepancy
reported below is wavelength-dependent rather than a general catalogue failure.}
...
the baseline \popcosmos{}/FSPS prediction. The model over-predicts in all three bands,
...
has the same sign in each independent survey-family benchmark, rises with wavelength, and exceeds the
...
The count over-prediction at long wavelengths means that the model is placing too much
energy past the SPIRE peak. To test this independently of the template experiment, the
...
dust, on the other hand the model slope is nearly flat ($\eta = +0.005$), and the peak is offset to
longer wavelengths (approximately \SI{126}{\um} vs \SI{92}{\um} at
$L_\mathrm{IR} = 10^{12}\,L_\odot$).}
\label{fig:drew_casey}
\end{figure}

\citet{Drew_2022} calibrated a quantitative relation with no redshift evolution out to
$z \sim 2$, finding
$\lambda_\mathrm{peak} \propto L_\mathrm{IR}^{-0.09}$, with
$\lambda_\mathrm{peak} \approx \SI{92}{\um}$ at
$L_\mathrm{IR} = 10^{12}\,L_\odot$. The \popcosmos{} model has two mismatches when compared to this benchmark. First, a normalisation offset. Model galaxies peak near \SI{126}{\um}
at the same luminosity, corresponding to colder dust. Second, and more importantly,
...
\caption{Left: predicted vs observed \SI{250}{\um} flux for matched galaxies, showing
the large per-object scatter. Right: among 422 model galaxies predicted above
\SI{20}{mJy}, only 21 are also observed above that threshold; 401 are pushed into
the bright regime just by scatter. But in a distribution with a falling source count, this promotion inflates the bright-end counts even when the median prediction is
too faint.}
\label{fig:scatter}
\end{figure}

Matched against the Wang catalogue, model galaxies show noise-deconvolved per-object
scatter of 0.45--\SI{0.51}{dex} (16--84th percentile half-width) at 250, 350 and
...
sources. Both statements are true simultaneously. To retain enough objects for the
threshold decomposition, the following calculation uses the wider
$\mathrm{SNR}\geq3$ sample. From the 422 model galaxies
...
\citep{B_thermin_2017,Negrello_2017}; it is contextual rather than a fit to the
...
already include magnified sources, whereas the model prediction applies no
magnification. If an illustrative 20\% lensed contribution is removed from the observed
...
Five potential concerns are addressed in conjunction.
...
negative as the cut is constricted, but the scatter does not decrease. If noise near the
detection limit were the reason for the broad distribution, the opposite of this would be expected.
...
10--\SI{100}{mJy}; as previously stated, the brighter bins are not used to make any claims.
Strong lensing becomes increasingly important at the bright 500~$\upmu$m end, but the
sensitivity test in Section~\ref{sec:results:valiante} shows that accounting for it
results in the excess increasing as opposed to improving.
...
useful total infrared luminosities, but in its current state, its far-infrared predictions are not reliable without having a better way to handle the dust-emission. Two separate issues are at play and they require different corrections.

The first is the population-level SED-shape. The baseline dust emission peaks
near \SI{136}{\um} in the rest frame and is nearly independent of luminosity. Because the dust peaks at longer wavelengths, large fractions of each galaxy's total energy ends up falling within the SPIRE bands. This raises the predicted 250, 350
and 500~$\upmu$m counts above those seen in any of the observed counts catalogues. Using warmer
dust shapes at fixed \LIR{} improves the count agreement, confirming that the shape is the issue.

The second is an object-level scatter issue. Individual predicted fluxes scatter by
0.42--\SI{0.51}{dex} against the Wang and Jin catalogues. Changing just the
the mean far-infrared template does not change this. Therefore just replacing the
template cannot solve both problems.

\subsection{Physical interpretation of the cold and uniform FIR shape}
\label{sec:discussion:cold}

Energy balance holds constant the integral of dust emission from 8 to \SI{1000}{\um}, but it
does not determine how that energy is distributed over wavelength. This work does not
show that \LIR{} is broken but that the default dust-shape assumptions in
\textsc{fsps} are too restrictive for far-infrared prediction.

The Drew and Casey diagnostic formalises this result \citep{Drew_2022}. Observed galaxies
...
peak wavelength is nearly flat across four decades of \LIR{}, and sit roughly
\SI{34}{\um} longer (colder) than the observed relation at
$L_\mathrm{IR} = 10^{12}\,L_\odot$. This missing luminosity to temperature dependence
puts forward an explanation for why the count excess is strongest at 350 and
500~$\upmu$m, where cold dust contributes the most flux.

In the Draine and Li framework, the missing trend points to the changes in the
radiation-field distribution rather than to the total energy. Increasing
...
The controlled template substitutions also support the same conclusion, but from a different angle. Modified blackbodies, ALESS hybrids and Casey-style models all move the
...
chi-square score should not be used considered a physical interpretation of the correct grain model.
...
The hot SED branch does not weaken this cold-dust conclusion because the two were
separated before fitting the peak relation. It is important for its methodological implications.
A median stacked SED remains cold even when a substantial fraction of its composing
galaxies have short-wavelength, AGN-associated peaks. Population stacks should therefore
be accompanied by distributions of individual SED properties. The association
between the hot branch and $f_\mathrm{AGN}$ is physically plausible, but its significance
...
galaxies say the model is typically too faint. Both are true due to shape of the source distribution.
...
The Wang--Jin agreement serves as a control of sorts. The two deblending catalogues agree with
...
agreement is a given cross-check as opposed to a fully independent validation. 

Thes implies that population summaries and per-object residuals test different
properties of the model. Matching counts alone could hide poor individual predictions,
and matching the median object alone could hide count inflation. Future validation across further regimes would require both to be considered.

\subsection{How strong are the conclusions?}
\label{sec:discussion:strength}

Several of these conclusions are quite robust. The sign of the baseline count excess is stable across
...
both Wang and Jin. These remain even under changes to the error floor, the source selection or
the bootstrap resampling.

Other conclusions are not as solid and remain tentative. The exact preferred temperature or template family
depends on the error floor and the flux range used. The significance of the
chi-square results are limited by correlated bins within each survey. The role of AGN
relies on the posterior medians of a parameter with a bimodal prior
...
bootstrap are tools to test the strength of the results as opposed to a full likelihood. The COSMOS area of
...
inference. These limitations weaken the selection of any particular model as opposed to disagreeing with the overlying result that the baseline far-infaredd treatment needs improvement. The range for reliable
count comparison is around 10--\SI{100}{mJy}.

\subsection{The Valiante bright-end uptick}
\label{sec:discussion:valiante}

The uptick in the brightest Valiante bins (Section~\ref{sec:results:valiante}) is seen
across all three bands and all three GAMA fields, so it is not a data handling,
area-weighting or unit-conversion error. \citet{Valiante_2016} report that the GAMA
fields differ most at their brightest flux densities and attribute this to cosmic
variance. Rare nearby galaxies, small-number statistics, gravitational lensing and the
count-inversion procedure may also contribute, but in this work these could not be easily separated.

Strong lensing is particularly relevant to very bright 500~$\upmu$m-selected samples
\citep{B_thermin_2017,Negrello_2017}. However, the Valiante band ratios do not show a
statistically measurable trend toward a stronger 500~$\upmu$m uptick. This does not rule
out a lensing contribution; it just means that these sparse bins cannot identify
one. More importantly for the model comparison, lensing cannot rescue the bright-count
excess which is the issue seen in the results. Because the observed corrected counts already contain lensed objects adding magnification to \popcosmos{} would increase its predicted bright counts worsening the disagreement.
...
dust template. This effect is negligible for the current COSMOS statistic as at
$\mathrm{SNR}\geq5$, the Wang catalogue contains one object above \SI{100}{mJy} at
250~$\upmu$m and none at 350 or 500~$\upmu$m. Regardless it should be treated
explicitly if the validation is extended to a much wider field. The conclusion that can be claimed here is to proceed with caution when interpreting results above \SI{100}{mJy}, rather than accepting a new population or lensing-based correction to the main result.


\subsection{Implications and recommended extension of \popcosmos{}}
\label{sec:discussion:recommendations}

Putting together the results for a set of potential recommendations for future \popcosmos{} work are as follows.

The immediate change would be to replace the fixed far-infrared bump with a
...
At the population level, a prior or some conditional relation between far-infrared shape and
properties such as \LIR{}, redshift and SFR should be introduced, this can be initially guided by
...
For validation, a having a calibrate then validate loop would help strengthen future claims.
This could be done by first calibrating on one set of independent survey fields and then evaluating on held-out fields.
Score differential counts by band and flux regime, and then separately report per-object bias
...
There could be improvements on the technical side by integrating through the true SPIRE bandpasses,
...
This work extends \popcosmos{} validation beyond its training regimes, identifies
where and why the extrapolation fails, and presents a reproducible
...
trained at optical and near-infrared wavelengths, can make reliable predictions in the
Herschel/SPIRE bands at 250, 350 and 500{\um}. The validation used
published differential number counts and matched COSMOS catalogues as directly observed
benchmarks, instead of relying on derived physical quantities. As expected the model works well where
it is constrained, but its current far-infrared extension does not reliably recreate
population counts or the individual galaxy fluxes.

Nine published count products from six papers were placed in common units and grouped
into three independent survey families, with overlapping products kept as sanity checks. The baseline \popcosmos{}/FSPS prediction over-predicts the counts in all three
...
The physical interpretation is that the baseline far-infrared dust bump is too cold and too
...
shows the direction needed (warmer and luminosity-dependent) but does not
...
0.45--\SI{0.51}{dex} against the Wang catalogue, while Wang and Jin agree with each
other to only 0.12--\SI{0.15}{dex}. The median matched galaxy is under-predicted, yet
the model still overproduces bright counts, because the steep source distribution and
large scatter push a lot more faint objects upward than bright objects downward.
Changing the mean dust template improves the counts but does not remove this large
object-to-object scatter. These are two different problems to address.

The excess maintains the same direction across all three independent survey families, under
...
of $+\SI{0.29}{dex}$ with an interval of $[+0.16, +0.52]$, treated as a way to test how robust this it as opposed to
a formal exclusion. The main limitations are correlated bins within
...
confirmed to be a feature of the published counts and therefore promotes caution above approximately
\SI{100}{mJy} as its origin was not confirmed.

The main recommendation for the \popcosmos{} project would be to preserve the learned
population model and energy balance, but add a galaxy-dependent far-infrared dust
...
available through CIGALE could be starting points. Future validation should calibrate
on one set of survey fields and test on held-out fields, reporting both differential
count performance and per-object flux scatter. The idea being that a generative
population model can produce physically sound parameters while still failing in
observables beyond its training regime. Validation beyond training regimes needs
...
ways: drafting and editing of sentences which were subsequently rewritten by
the author. Cross-checking of numerical results reported in
Section~\ref{sec:results} and LaTeX formatting. 
